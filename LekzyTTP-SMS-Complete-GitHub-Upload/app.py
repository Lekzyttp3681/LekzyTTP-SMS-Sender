...
try:
    ...
    # Create fallback managers that handle errors gracefully
    user_manager = None
    admin_manager = None
...
def admin_authenticate():
    ...
    try:
        if admin_manager is None:
            return render_template('admin_login.html', error='Database connection unavailable')
        ...
        return render_template('admin_login.html', error='Authentication system unavailable')
...
def admin_dashboard():
    ...
    try:
        if admin_manager is None or user_manager is None:
            return "Database connection unavailable. Please check Railway PostgreSQL connection.", 503
